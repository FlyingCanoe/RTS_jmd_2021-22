...
class Vue():
    ...
    def demanderabandon(self):
        rep = messagebox.askyesno("Abandonner", "Vous desirez VRAIMENT abandonner la partie?")
        if rep:
            self.parent.abandonner()
            # self.root.destroy()
    ...
    def initialiseravecmodele(self):
        ...
        self.creeraide()
        self.creercadreouvrier(coul[0] + "_", ["maison", "caserne", "abri", "usineballiste"])
        self.creerchatter()
    ...
    def chasserressource(self, evt):
        tag = self.canevas.gettags(CURRENT)
        # self.action.persochoisi vérifie si un uniter est selectioner
    ...
    def creerentite(self, evt):
        ...
        if self.parent.monnom in mestags and "batiment" in mestags:
            ...
            if "abri" in mestags:
                pos = (self.canevas.canvasx(evt.x), self.canevas.canvasy(evt.y))
                action = [self.parent.monnom, "creerperso", ["druide", mestags[4], mestags[1], pos]]
        ...
        for i in self.modele.listebiotopes:
            if i.montype == "daim":
                monitem = self.canevas.create_image(i.x, i.y, image=self.images[i.img], anchor=S,
                                                    tags=("", i.id, "artefact", i.montype, "mobile"))
            else:
                monitem = self.canevas.create_image(i.x, i.y, image=self.images[i.img], anchor=S,
                                                    tags=("", i.id, "artefact", i.montype))
    ...
    def afficherjeu(self):
        ...
        listedaimvisibles=[]
        for i in mesitems:
            lestags=self.canevas.gettags(i)
            if "daim" in lestags:
                listedaimvisibles.append([i,lestags[1]])
        for k,j in listedaimvisibles:
            i=self.modele.biotopes["daim"][j]
            if i.etat=="mort":
                self.canevas.delete(k)
                self.canevas.create_image(i.x,i.y,image=self.images["daimMORT"],tags=("",i.id,"artefact","daim"))

            else:
                self.canevas.move(k,i.x,i.y)
                print(j,i.x,i.y)
        """
